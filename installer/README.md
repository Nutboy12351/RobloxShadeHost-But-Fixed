...
Requires Inno Setup 6. The installer includes RobloxShadeHost and offers ReShade with full add-on support and the optional DLSS5 add-on. Presets will be added separately.
...
To install ReShade unattended, first read its license and explicitly pass `/ACCEPTRESHADELICENSE=1`. Select `host,reshade` or `host,reshade,reshade\dlss5` with `/COMPONENTS`. Use `/LOG="setup.log"` to record download failures.

Uninstall removes installed binaries and the shortcut. It retains ReShade settings and files created later by the user.

## Verification

Run `./tests/installer_tests.ps1` after building the host. The tests compile an installer without uninstall registration or shortcuts and install into fresh folders under `build/installer-tests`. They check component selection, license acceptance, configuration preservation, and unavailable DLSS5 downloads. Internet access is required for ReShade.
